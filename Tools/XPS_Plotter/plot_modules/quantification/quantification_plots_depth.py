...
def plot_atomic_concentration_per_sample(
    ...
    ax.set_title(f"{sample}: Atomic Concentration Across Layers", fontsize=font_config['title_size'], fontweight='bold')
...
def plot_atomic_concentration_layer_comparison(
    ...
    ax.tick_params(axis="x", labelsize=font_config['tick_label_size'])
    ax.tick_params(axis="y", labelsize=font_config['tick_label_size'])
    ax.set_title(f"Layer {layer}: Atomic Concentration Across Samples", fontsize=font_config['title_size'], fontweight='bold')
    ax.grid(axis="y", linestyle="--", alpha=plot_config['lines']['grid_alpha'])
    ax.legend(ncol=2, fontsize=font_config['legend_size'], frameon=False)
    fig.tight_layout()
...
def plot_chemistry_heatmap(chemistry_name, component_data_dict, plots_dir, config=None):
    ...
    font_config = plot_config['fonts']
    ax.set_title(f'{chemistry_name} Component Distribution Across Layers', 
...
def plot_depth_profile_3d_waterfall(
    ...
    ax.set_ylabel("Layer (Depth)", fontsize=font_cfg['axis_label_size'], labelpad=14)
    ax.set_zlabel("Intensity (cps)", fontsize=font_cfg['axis_label_size'], labelpad=14)

    ax.set_title(
        f"3D Waterfall - {region} Across Layers\n({depth_title}, rotate to explore)",
    ...
    try:
        ...
        print(f"⚠️  Error saving 3D waterfall plot: {exc}")
        plt.close(fig)
        return None
...
def load_depth_profile_csv(csv_path: Path) -> Optional[Dict[int, tuple[np.ndarray, np.ndarray]]]:
    ...
    try:
        ...
        print(f"⚠️  Error loading depth profile CSV: {exc}")
        return None
...
def plot_quantification_overview(quantification_results, plots_dir, config=None):
    ...
    font_config = plot_config['fonts']
    fig.suptitle('XPS Quantification Overview (Depth Profile)', 
                fontsize=font_config['title_size'], fontweight='bold')
...
def plot_element_cross_sample_distribution(
    ...
    ax.set_ylabel("Atomic Concentration (%)", fontsize=font_config['axis_label_size'])
    ax.set_title(
        f"{element_label}: Atomic Concentration Across Samples by Layer",
...
def generate_component_chemistry_plots(
    ...
    for chem_name, chem_group in chemistry_groups.items():
        ...
        ax.set_xlabel('Sample_Layer', fontsize=font_cfg['axis_label_size'], fontweight='bold')
        ax.set_ylabel('Component Atomic %', fontsize=font_cfg['axis_label_size'], fontweight='bold')
        ax.set_title(
            f'{chem_name}: Component Distribution Across Layers',
            fontsize=font_cfg['title_size'],
            fontweight='bold',
        )
...
def _create_individual_component_trends(
    ...
    for idx, component in enumerate(components):
        ...
        ax.set_xlabel('Sample_Layer', fontsize=font_cfg['axis_label_size']-2)
    ...
    fig.suptitle(
        f'{chem_name}: Individual Component Trends Across Layers',
        fontsize=font_cfg['title_size'],
        fontweight='bold',
        y=0.995,
    )
...
# 3D Waterfall plot merged into this depth-profile module.
