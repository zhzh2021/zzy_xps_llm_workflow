"""
Stage 06 - MLP Regression.
Trains a regression MLP on feature-based datasets.
"""
...
from xas_deep_learning.dl_models import build_mlp
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train MLP regression model.")
    ...
    model_cfg = cfg.get("models", {}).get("mlp_regression", {})
    ...
    run_name = f"mlp_regression_{run_id}"
    ...
    hidden_layers = model_cfg.get("hidden_layers", [512, 256, 128, 64, 32])
    l2 = float(model_cfg.get("l2", 0.0001))
    ...
    plot_training_history(history, plot_dir / "training_loss.png", "MLP Regression Loss")
    plot_regression_results(y_test_eval, y_pred, plot_dir / "prediction_scatter.png", "MLP Regression")
    plot_residuals(y_test_eval, y_pred, plot_dir / "residuals.png", "MLP Regression Residuals")
    plot_error_hist(y_test_eval, y_pred, plot_dir / "residual_hist.png", "MLP Regression Residuals")
